...
# HTTP_PORT must not be the same as HTTPS_PORT (if they are both defined)
MACROS =	$(if $(HTTP_PORT),-D'HTTP_PORT=$(HTTP_PORT)')\
		$(if $(HTTPS_PORT),-D'HTTPS_PORT=$(HTTPS_PORT)')\
		$(if $(TIMEOUT),-D'TIMEOUT=$(TIMEOUT)')\
		$(if $(TIMEOUT_USEC),-D'TIMEOUT_USEC=$(TIMEOUT_USEC)')
